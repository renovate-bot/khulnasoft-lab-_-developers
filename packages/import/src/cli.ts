...
import { aiexecCsvImporter } from "./importers/aiexecCsv";
  ...
  try {
    const importAnswers = await inquirer.prompt<ImportAnswers>([
      {
        type: "input",
        name: "aiexecApiKey",
        message: "Input your Aiexec API key (https://aiexec.khulnasoft.com/settings/account/security)",
      },
      {
        type: "list",
        name: "service",
    ...
    switch (importAnswers.service) {
      ...
      case "trelloJson":
        ...
        break;
      case "aiexecCsv":
        importer = await aiexecCsvImporter();
        break;
      default:
    ...
    if (importer) {
      ...
      await importIssues(importAnswers.aiexecApiKey, importer, apiUrl);
    }
  } catch (error) {
