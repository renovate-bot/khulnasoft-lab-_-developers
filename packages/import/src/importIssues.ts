...
export const importIssues = async (apiKey: string, importer: Importer, apiUrl?: string): Promise<void> => {
  ...
  console.info(
    chalk.green(`${importer.name} issues imported to your team: https://aiexec.khulnasoft.com/team/${teamKey}/all`)
  );
};
...
const createIssueWithRetries = async (
  client: AiexecClient,
  input: Parameters<AiexecClient["createIssue"]>[0],
  retries = 3
): ReturnType<AiexecClient["createIssue"]> => {
  try {
