...
describe("readme.md", () => {
  /** Initialize Aiexec client variable */
  let aiexecClient: AiexecClient;
  ...
  beforeAll(async () => {
    aiexecClient = await startClient();
  });
  ...
  async function createTeam(input: AiexecDocument.TeamCreateInput) {
    aiexecClient.createTeam(input);
  }
    ...
    describe("Query for your issues", () => {
      it("Using async await syntax", async () => {
        ...
        async function getMyIssues() {
          const me = await aiexecClient.viewer;
  ...
  describe("Typescript", () => {
    it("All types are accessible through the Aiexec Client package. It is written in Typescript", async () => {
      /** CODE_SECTION:2_1:START */
      // CODE_SECTION:INCLUDE import { AiexecClient, AiexecFetch, User } from "@aiexec/sdk";

      const aiexecClient = new AiexecClient({ apiKey });

      async function getCurrentUser(): AiexecFetch<User> {
        return aiexecClient.viewer;
      }
  ...
  describe("Query", () => {
    it("Some models can be fetched from the Aiexec Client without any arguments", async () => {
      /** CODE_SECTION:3_1:START */
      const me = await aiexecClient.viewer;
      const org = await aiexecClient.organization;
      /** CODE_SECTION:3_1:END */
    });

    it("Other models are exposed as connections, and return a list of nodes", async () => {
      /** CODE_SECTION:3_2:START */
      const issues = await aiexecClient.issues();
    ...
    it("All required variables are passed as the first arguments", async () => {
      ...
      const user = await aiexecClient.user("user-id");
      const team = await aiexecClient.team("team-id");
      /** CODE_SECTION:3_3:END */
    });

    it("Any optional variables are passed into the last argument as an object", async () => {
      /** CODE_SECTION:3_4:START */
      const fiftyProjects = await aiexecClient.projects({ first: 50 });
      const allComments = await aiexecClient.comments({ includeArchived: true });
  ...
  describe("Mutate", () => {
    it("To create a model, call the Aiexec Client mutation and pass in the input object", async () => {
      /** CODE_SECTION:4_1:START */
      const teams = await aiexecClient.teams();
      const team = teams.nodes[0];
      if (team.id) {
        await aiexecClient.createIssue({ teamId: team.id, title: "My Created Issue" });
      }
      /** CODE_SECTION:4_1:END */
    });

    it("To update a model, call the Aiexec Client mutation and pass in the required variables and input object", async () => {
      /** CODE_SECTION:4_2:START */
      const me = await aiexecClient.viewer;
      if (me.id) {
        await aiexecClient.updateUser(me.id, { displayName: "Alice" });
      }
    ...
    it("All mutations are exposed in the same way", async () => {
      ...
      const projects = await aiexecClient.projects();
      const project = projects.nodes[0];
      if (project.id) {
        await aiexecClient.archiveProject(project.id);
    ...
    it("Mutations will often return a success boolean and the mutated entity", async () => {
      ...
      const commentPayload = await aiexecClient.createComment({ issueId: "some-issue-id" });
  ...
  describe("Paginate", () => {
    it("Connection models have helpers to fetch the next and previous pages of results", async () => {
      ...
      const issues = await aiexecClient.issues({ after: "some-issue-cursor", first: 10 });
    ...
    it("Pagination info is exposed and can be passed to the query operations. This uses the [Relay Connection spec](https://relay.dev/graphql/connections.htm)", async () => {
      ...
      const moreIssues = await aiexecClient.issues({ after: issuesEndCursor, first: 10 });
      /** CODE_SECTION:5_2:END */
    });

    it("Results can be ordered using the `orderBy` optional variable", async () => {
      /** CODE_SECTION:5_3:START */
      // CODE_SECTION:INCLUDE import { AiexecDocument } from "@aiexec/sdk";

      const issues = await aiexecClient.issues({ orderBy: AiexecDocument.PaginationOrderBy.UpdatedAt });
  ...
  describe("File Upload", () => {
    it("Create a file upload URL, upload the file to external storage, and attach the file by asset URL", async () => {
      ...
      // CODE_SECTION:INCLUDE import { Issue, AiexecFetch } from "@aiexec/sdk";

      async function createIssueWithFile(
        title: string,
        file: File,
        uploadData: RequestInit
      ): AiexecFetch<Issue | undefined> {
        /** Fetch a storage URL to upload the file to */
        const uploadPayload = await aiexecClient.fileUpload(file.type, file.name, file.size);
        ...
        if (uploadUrl && authHeader?.key && authHeader?.value) {
          ...
          if (assetUrl) {
            const issuePayload = await aiexecClient.createIssue({
              title,
  ...
  describe("Error", () => {
    it("Errors can be caught and interrogated by wrapping the operation in a try catch block", async () => {
      ...
      async function createComment(
        input: AiexecDocument.CommentCreateInput
      ): AiexecFetch<Comment | undefined | UserError> {
        try {
          /** Try to create a comment */
          const commentPayload = await aiexecClient.createComment(input);
          /** Return it if available */
          return commentPayload.comment;
        } catch (error) {
          /** The error has been parsed by Aiexec Client */
          throw error;
        }
      }
    ...
    it("Or by catching the error thrown from a calling function", async () => {
      ...
      async function archiveFirstIssue(): AiexecFetch<ArchivePayload | undefined> {
        const me = await aiexecClient.viewer;
        const issues = await me.assignedIssues();
        const firstIssue = issues.nodes[0];

        if (firstIssue.id) {
          const payload = await aiexecClient.archiveIssue(firstIssue.id);
          return payload;
        } else {
          return undefined;
        }
    ...
    it("The parsed error type can be compared to determine the course of action", async () => {
      ...
      // CODE_SECTION:INCLUDE import { InvalidInputAiexecError, AiexecError, AiexecErrorType } from '@aiexec/sdk'
      // CODE_SECTION:INCLUDE import { UserError } from './custom-errors'

      const input = { name: "Happy Team" };
      createTeam(input).catch(error => {
        if (error instanceof InvalidInputAiexecError) {
      ...
      run().catch(error => {
        if (error instanceof AiexecError) {
          error.errors?.map(graphqlError => {
            ...
            console.log("AiexecErrorType of this GraphQL error", graphqlError.type);
    ...
    it("The `raw` error returned by the AiexecGraphQLClient is still available", async () => {
  ...
  describe("🌊 Advanced", () => {
    it("Request Configuration", () => {
      ...
      const aiexecClient = new AiexecClient({ apiKey, headers: { "my-header": "value" } });
      /** CODE_SECTION:8_1:END */
    });

    it("Raw GraphQL Client", () => {
      /** CODE_SECTION:8_2:START */
      const graphQLClient = aiexecClient.client;
    ...
    it("Custom GraphQL Client", () => {
      ...
      // CODE_SECTION:INCLUDE import { AiexecError, AiexecFetch, AiexecRequest, AiexecSdk, parseAiexecError, UserConnection } from "@aiexec/sdk";
      ...
      /** Create a custom client configured with the Aiexec API base url and API key */
      const customGraphqlClient = new CustomGraphqlClient("https://api.aiexec.khulnasoft.com/graphql", {
        headers: { Authorization: apiKey },
      });

      /** Create the custom request function */
      const customAiexecRequest: AiexecRequest = <Data, Variables>(document: DocumentNode, variables?: Variables) => {
        ...
        return customGraphqlClient.request<Data, Variables>(print(document), variables).catch(error => {
          /** Optionally catch and parse errors from the Aiexec API */
          throw parseAiexecError(error);
        });
      };

      /** Extend the Aiexec SDK to provide a request function using the custom client */
      class CustomAiexecClient extends AiexecSdk {
        public constructor() {
          super(customAiexecRequest);
        }
      }

      /** Create an instance of the custom client */
      const customAiexecClient = new CustomAiexecClient();

      /** Use the custom client as if it were the Aiexec Client */
      async function getUsers(): AiexecFetch<UserConnection> {
        const users = await customAiexecClient.users();
        return users;
      }
