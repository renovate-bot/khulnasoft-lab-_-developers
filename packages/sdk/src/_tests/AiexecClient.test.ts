...
describe("AiexecClient", () => {
  ...
  it("makes query to apiUrl", async () => {
    const client = new AiexecClient({ apiKey: MOCK_API_KEY, apiUrl: ctx.url });
  ...
  it("supports overriding headers", async () => {
    const client = new AiexecClient({ apiKey: MOCK_API_KEY, apiUrl: ctx.url, headers: { user: "asd" } });
    const response = await client.viewer;
  ...
  it("fails auth with incorrect api key", async () => {
    const client = new AiexecClient({ apiKey: "asd", apiUrl: ctx.url });

    try {
      await client.viewer;
    } catch (error) {
      expect(error.message).toEqual(expect.stringContaining("GraphQL Error (Code: 401) - Unauthorized"));
      expect(error.type).toEqual(AiexecErrorType.AuthenticationError);
    }
  });

  it("rawRequest fails auth with incorrect api key", async () => {
    const client = new AiexecClient({ apiKey: "asd", apiUrl: ctx.url }).client;
