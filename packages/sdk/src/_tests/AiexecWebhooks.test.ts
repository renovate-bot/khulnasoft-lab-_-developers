...
import { AiexecWebhookClient, AIEXEC_WEBHOOK_TS_FIELD } from "../webhooks";
...
describe("webhooks", () => {
  ...
  beforeEach(() => {
    requestBody = {
      ...
      type: "Comment",
      url: "https://aiexec.khulnasoft.com/issue/LIN-1778/foo-bar#comment-77217de3-fb52-4dad-bb9a-b356beb93de8",
      createdAt: "2020-01-23T12:53:18.084Z",
  ...
  it("incorrect signature, should fail verification", async () => {
    const webhook = new AiexecWebhookClient("SECRET");
  ...
  it("correct signature, incorrect timestamp should fail verification", async () => {
    ...
    expect(() => webhook.verify(rawBody, signature, parsedBody[AIEXEC_WEBHOOK_TS_FIELD])).toThrowError(
  ...
  it("correct signature, correct timestamp should pass verification", async () => {
    ...
    const webhook = new AiexecWebhookClient("SECRET");
    const signature = crypto.createHmac("sha256", "SECRET").update(rawBody).digest("hex");
    expect(() => webhook.verify(rawBody, signature, parsedBody[AIEXEC_WEBHOOK_TS_FIELD])).toBeTruthy();
  });
});
