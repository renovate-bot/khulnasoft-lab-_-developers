import * as es from "@aiexec/sdk";
import { AiexecErrorType } from "../types";
 ...
 */
async function expectError(shouldError: () => unknown, type: AiexecErrorType, message: string) {
  try {
    await shouldError();
  } catch (error) {
...
Object.entries(bundles).map(([bundleFormat, bundle]) =>
  describe(bundleFormat, () => {
    const ClientConstructor = bundle?.AiexecClient;

    /** Initialize Aiexec client variable */
    let aiexecClient: es.AiexecClient;

    beforeAll(async () => {
      if (ClientConstructor) {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        aiexecClient = (await startClient(ClientConstructor as any)) as unknown as es.AiexecClient;
      } else {
        throw new Error(`Bundle for ${bundleFormat} has no AiexecClient`);
      }
    });
    ...
    async function getSomeTeam() {
      ...
      const team = await aiexecClient.team(first?.id ?? "");
    ...
    async function getSomeIssue() {
      ...
      const issue = await aiexecClient.issue(first?.id ?? "");
    ...
    describe("queries", () => {
      it("query for the viewer", async () => {
        const viewer = await aiexecClient.viewer;
      ...
      it("query for fake team", async () => {
        expectError(
          () => aiexecClient.team("not a real team id"),
          AiexecErrorType.InvalidInput,
      ...
      it("query for fake issue", async () => {
        expectError(
          () => aiexecClient.issue("not a real issue id"),
          AiexecErrorType.InvalidInput,
