...
import { GraphQLRequestContext, AiexecRawResponse } from "./types";

/**
 * Identical class to graphql-request ClientError
 * Ensures parseAiexecError is compatible with custom graphql-request clients
 *
 * @param response the raw response from the Aiexec API
...
export class GraphQLClientError<Data, Variables extends Record<string, unknown>> extends Error {
  public response: AiexecRawResponse<Data>;
  public request: GraphQLRequestContext<Variables>;

  public constructor(response: AiexecRawResponse<Data>, request: GraphQLRequestContext<Variables>) {
  ...
  private static extractMessage(response: AiexecRawResponse<unknown>): string {
    try {
 ...
 */
export class AiexecGraphQLClient {
  private url: string;
  ...
  ): Promise<AiexecRawResponse<Data>> {
  ...
  public setHeaders(headers: RequestInit["headers"]): AiexecGraphQLClient {
   ...
   */
  public setHeader(key: string, value: string): AiexecGraphQLClient {
 ...
 *
 * @param response raw response from the Aiexec API
 */
function getResult<Data>(response: Response): Promise<AiexecRawResponse<Data> | string> {
