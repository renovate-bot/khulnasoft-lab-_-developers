...
- copy `test_files/config_template.json` into `test_files/config.json` and replace `${AIEXEC_AIRBYTE_KEY}` with an integration key for your workspace. See https://aiexec.khulnasoft.com/docs/airbyte for more details
- run `docker pull airbyte/source-acceptance-test` to download airbyte's docker image with integration test suite
- build source connector docker image `docker build -t airbyte-source .`
