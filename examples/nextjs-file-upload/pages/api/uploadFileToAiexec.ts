...
import { AiexecClient } from "@aiexec/sdk";
import { NextFunction } from "express";
import { IncomingMessage, ServerResponse } from "http";

const aiexecClient = new AiexecClient({ apiKey: process.env.AIEXEC_API_KEY });
...
export default async function (req: NextApiRequest, res: NextApiResponse): Promise<void> {
  ...
  try {
    const url = await uploadFileToAiexec(multerFile);
...
/** Uploads a file to Aiexec, returning the uploaded URL. */
async function uploadFileToAiexec(file: Express.Multer.File): Promise<string> {
  const uploadPayload = await aiexecClient.fileUpload(file.mimetype, file.originalname, file.size);
  ...
  try {
    ...
    throw new Error("Failed to upload file to Aiexec", { cause: e });
  }
}
